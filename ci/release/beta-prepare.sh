...
standard-version -t wrapland\@ --skip.commit true --skip.tag true --preMajor --release-as $BETA_VERSION
...
git tag -a "wrapland@${BETA_VERSION}" -m "Create beta release ${BETA_VERSION}."

# Now checkout the next stable branch.
BRANCH_NAME="Plasma/$(echo $NEXT_VERSION | sed -e 's,^\w.\(\w\)\(\w*\).*,\1.\2,g')"
