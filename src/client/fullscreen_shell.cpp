...
private:
    ...
    static void
    capabilitiesAnnounce(void* data, struct _wl_fullscreen_shell* shell, uint32_t capability);
...
void FullscreenShell::Private::capabilitiesAnnounce(void* data,
                                                    _wl_fullscreen_shell* shell,
                                                    uint32_t capability)
{
...
{
    Q_ASSERT(d->shell);
    _wl_fullscreen_shell_present_surface(
        d->shell, surface, _WL_FULLSCREEN_SHELL_PRESENT_METHOD_DEFAULT, output);
}
