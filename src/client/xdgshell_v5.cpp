...
XdgShellPopup* XdgShellUnstableV5::Private::getXdgPopup(Surface* surface,
                                                        Surface* parentSurface,
                                                        Seat* seat,
                                                        quint32 serial,
                                                        const QPoint& parentPos,
                                                        QObject* parent)
{
    ...
    auto w = zxdg_shell_v5_get_xdg_popup(
        xdgshellv5, *surface, *parentSurface, *seat, serial, parentPos.x(), parentPos.y());
    if (queue) {
        queue->addProxy(w);
    }
    s->setup(w);
...
const struct zxdg_surface_v5_listener XdgShellSurfaceUnstableV5::Private::s_listener
    = {configureCallback, closeCallback};
...
void XdgShellSurfaceUnstableV5::Private::showWindowMenu(Seat* seat,
                                                        quint32 serial,
                                                        qint32 x,
                                                        qint32 y)
{
