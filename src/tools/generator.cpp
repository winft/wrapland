        ...
        for (auto it = m_interfaces.begin(); it != m_interfaces.end(); ++it) {
            ...
            for (auto r : (*it).requests()) {
                for (auto a : r.arguments()) {
                    if (a.type() == Argument::Type::NewId
                        && a.interface().compare(interfaceName) == 0) {
                        return &(*it);
                    }
                }
            }
...
void Generator::startAuthorNameProcess()
{
    ...
    git->setArguments(QStringList{
        QStringLiteral("config"), QStringLiteral("--get"), QStringLiteral("user.name")});
...
void Generator::startAuthorEmailProcess()
{
    QProcess* git = new QProcess(this);
    git->setArguments(QStringList{
        QStringLiteral("config"), QStringLiteral("--get"), QStringLiteral("user.email")});
    git->setProgram(QStringLiteral("git"));
    connect(git,
            static_cast<void (QProcess::*)(int, QProcess::ExitStatus)>(&QProcess::finished),
            this,
...
{
    const QString templateString = QStringLiteral("#pragma once\n\n");
...
void Generator::generateServerPrivateHeaderGlobalClass(Interface const& interface)
{
    QString templateString = QStringLiteral(
        ...
        "\n");
    *m_stream.localData()
        << templateString.arg(interface.wraplandServerName()).arg(interface.version());
...
void Generator::generateServerPrivateGlobalConstructor(Interface const& interface)
{
    QString templateString = QStringLiteral(
        ...
        "\n");
    *m_stream.localData()
        << templateString.arg(interface.wraplandServerName()).arg(interface.name());
    ...
    for (auto const& r : requests) {
        ...
        for (auto const& a : r.arguments()) {
            ...
            if (a.type() == Argument::Type::Object) {
                arguments.append(
                    QStringLiteral("%1 *%2").arg(a.typeAsQt()).arg(toCamelCase(a.name())));
            } else {
                arguments.append(
                    QStringLiteral("%1 %2").arg(a.typeAsQt()).arg(toCamelCase(a.name())));
            }
        ...
        if (factored.isEmpty()) {
            ...
            *m_stream.localData() << factoryTemplateString.arg(toQtInterfaceName(factored))
                                         .arg(toCamelCase(r.name()))
                                         .arg(arguments);
        }
    }
...
int main(int argc, char** argv)
{
    ...
    QCommandLineOption fileName(
        ...
        QStringLiteral("The base name of files to be generated. E.g. for \"foo\" the files "
                       "\"foo.h\" and \"foo.cpp\" are generated."
