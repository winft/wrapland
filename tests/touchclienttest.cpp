...
void WaylandClientTest::setupRegistry(Registry* registry)
{
    ...
    connect(registry, &Registry::seatAnnounced, this, [this, registry](quint32 name) {
        ...
        connect(s, &Seat::hasKeyboardChanged, this, [this, s](bool has) {
            ...
            connect(k, &Keyboard::keyChanged, this, [this](quint32 key, Keyboard::KeyState state) {
        ...
        connect(s, &Seat::hasTouchChanged, this, [this, s](bool has) {
            ...
            connect(t, &Touch::sequenceStarted, this, [](Wrapland::Client::TouchPoint* startPoint) {
                qDebug() << "Touch sequence started at" << startPoint->position() << "with id"
                         << startPoint->id();
            });
            connect(
                t, &Touch::sequenceCanceled, this, []() { qDebug() << "Touch sequence canceled"; });
            connect(
                t, &Touch::sequenceEnded, this, []() { qDebug() << "Touch sequence finished"; });
            connect(t, &Touch::frameEnded, this, []() {
                qDebug() << "End of touch contact point list";
            });
            connect(t, &Touch::pointAdded, this, [](Wrapland::Client::TouchPoint* point) {
                qDebug() << "Touch point added at" << point->position() << "with id" << point->id();
            });
            connect(t, &Touch::pointRemoved, this, [](Wrapland::Client::TouchPoint* point) {
                qDebug() << "Touch point " << point->id() << " removed at" << point->position();
            });
            connect(t, &Touch::pointMoved, this, [](Wrapland::Client::TouchPoint* point) {
