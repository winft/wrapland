...
static QLayout*
setupOutput(Registry::AnnouncedInterface outputInterface, Registry* registry, DpmsManager* manager)
{
    Output* output
    ...
    QObject::connect(
        ...
        &Output::changed,
        label,
        [label, output] { label->setText(output->model()); },
        Qt::QueuedConnection);
    ...
    if (dpms) {
        ...
        QObject::connect(
            ...
            &Dpms::modeChanged,
            modeLabel,
            [modeLabel, dpms] { modeLabel->setText(modeToString(dpms->mode())); },
            Qt::QueuedConnection);
        ...
        QObject::connect(
            offButton, &QPushButton::clicked, dpms, [dpms] { dpms->requestMode(Dpms::Mode::Off); });
    }
